#!/usr/bin/env python3
"""
Comprehensive FYNDR Parameter Analysis

This script runs comprehensive 270-day simulations to find optimal parameter ranges.
"""
...
def run_comprehensive_test(config_name, config_params, days=270):
    """Run a comprehensive test with given configuration"""
    try:
...
def create_test_configurations():
    ...
    # Pack pricing tests
    pricing_tests = [
        ("Low Price, Low Points", {**base_config, 'pack_price_dollars': 2.0, 'pack_price_points': 200}),
        ("Low Price, Med Points", {**base_config, 'pack_price_dollars': 2.5, 'pack_price_points': 250}),
        ("Base Price, Base Points", {**base_config, 'pack_price_dollars': 3.0, 'pack_price_points': 300}),
        ("Base Price, Low Points", {**base_config, 'pack_price_dollars': 3.0, 'pack_price_points': 250}),
        ("Base Price, High Points", {**base_config, 'pack_price_dollars': 3.0, 'pack_price_points': 350}),
        ("High Price, Base Points", {**base_config, 'pack_price_dollars': 3.5, 'pack_price_points': 300}),
        ("High Price, High Points", {**base_config, 'pack_price_dollars': 4.0, 'pack_price_points': 400}),
    ]
    
    # Scoring mechanics tests
    scoring_tests = [
        ("Low Base Scoring", {**base_config, 'owner_base_points': 1.5, 'scanner_base_points': 0.75}),
        ("Base Scoring", {**base_config, 'owner_base_points': 2.0, 'scanner_base_points': 1.0}),
        ("High Base Scoring", {**base_config, 'owner_base_points': 2.5, 'scanner_base_points': 1.25}),
        ("Very High Base Scoring", {**base_config, 'owner_base_points': 3.0, 'scanner_base_points': 1.5}),
        ("Low Unique Bonus", {**base_config, 'unique_scanner_bonus': 0.5}),
        ("High Unique Bonus", {**base_config, 'unique_scanner_bonus': 1.5}),
        ("Very High Unique Bonus", {**base_config, 'unique_scanner_bonus': 2.0}),
    ]
    
    # Diversity bonus tests
    diversity_tests = [
        ("Low Diversity Bonuses", {**base_config, 'geo_diversity_bonus': 0.5, 'venue_variety_bonus': 0.5}),
        ("Base Diversity Bonuses", {**base_config, 'geo_diversity_bonus': 1.0, 'venue_variety_bonus': 1.0}),
        ("High Diversity Bonuses", {**base_config, 'geo_diversity_bonus': 1.5, 'venue_variety_bonus': 1.5}),
        ("Very High Diversity Bonuses", {**base_config, 'geo_diversity_bonus': 2.0, 'venue_variety_bonus': 2.0}),
        ("Small Geo Radius", {**base_config, 'geo_diversity_radius': 300.0}),
        ("Base Geo Radius", {**base_config, 'geo_diversity_radius': 500.0}),
        ("Large Geo Radius", {**base_config, 'geo_diversity_radius': 750.0}),
        ("Very Large Geo Radius", {**base_config, 'geo_diversity_radius': 1000.0}),
    ]
    
    # Engagement cap tests
    engagement_tests = [
        ("Low Scan Cap", {**base_config, 'daily_scan_cap': 10}),
        ("Base Scan Cap", {**base_config, 'daily_scan_cap': 20}),
        ("High Scan Cap", {**base_config, 'daily_scan_cap': 30}),
        ("Low Weekly Cap", {**base_config, 'weekly_earn_cap': 300}),
        ("Base Weekly Cap", {**base_config, 'weekly_earn_cap': 500}),
        ("High Weekly Cap", {**base_config, 'weekly_earn_cap': 800}),
    ]
    
    tests.extend(pricing_tests)
    tests.extend(scoring_tests)
    tests.extend(diversity_tests)
    tests.extend(engagement_tests)
    
    return tests

def main():
    """Run comprehensive analysis"""
    print("Comprehensive FYNDR Parameter Analysis")
    print("=" * 50)
    print("Running 270-day simulations to find optimal parameter ranges...")
    print()
    
    # Create test configurations
    tests = create_test_configurations()
    ...
    for i, (test_name, config_params) in enumerate(tests):
        ...
        result = run_comprehensive_test(test_name, config_params, 270)
        if result:
    ...
    with open('comprehensive_analysis_results.csv', 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(['name', 'overall_score', 'growth_score', 'retention_score', 'organic_purchase_score', 'players', 'revenue', 'avg_points', 'avg_scans', 'whale_revenue', 'grinder_revenue', 'casual_revenue', 'organic_revenue'])
        for result in results:
            writer.writerow([
                result['name'],
                ...
                result['avg_points'],
                result['avg_scans'],
                result['whale_revenue'],
                result['grinder_revenue'],
                result['casual_revenue'],
                result['organic_revenue']
            ])
    
    print(f"\nResults exported to comprehensive_analysis_results.json and comprehensive_analysis_results.csv")
    ...
    scoring_results = [r for r in top_results if 'Scoring' in r['name'] or 'Bonus' in r['name']]
    if scoring_results:
        ...
        print(f"  • Unique Scanner Bonus: {best_scoring['config']['unique_scanner_bonus']:.1f}")
        print(f"  • Avg Points/Player: {best_scoring['avg_points']:.1f}")
    
    # Diversity bonuses analysis
    diversity_results = [r for r in top_results if 'Diversity' in r['name'] or 'Radius' in r['name']]
    if diversity_results:
        ...
        print(f"  • Geo Diversity Radius: {best_diversity['config']['geo_diversity_radius']:.0f}m")
